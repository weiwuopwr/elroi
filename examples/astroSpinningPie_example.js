...
var astroSpinningPie = (function($){
    ...
    var wedgeAnimationDuration = 100,
    ...
    /**
     * Update the message text by providing a wedge index.
     * @param index {number} index of wedge in wedges to use to update the message text.
     */
    function updateMessageText(index){
    ...
    function surveyInitialize(callback) {
        pie.rotate(-90, function(){
            ...
            /* Continuously show message box */
            updateMessageText(sectionsCompleted);
    ...
    function wedgeHoverIn(e, wedge) {
        ...
        if(circle.node !== e.fromElement) {
            updateMessageText(pie.getWedgeIndex(wedge));
        }
    ...
    function wedgeHoverOut(e, wedge) {
        ...
        /* If we're in any mode except survey mode, hide the message box */
        if(mode !== Mode.SURVEY) {
    ...
    ns.changeSection = function(section,series) {
        //Already in the right mode and section, don't need to take any further action.
        if(sectionsCompleted === section && Mode.SURVEY === mode){
            return;
        }
    ...
    ns.changeMode = function(newMode, series){
        ...
        /* New Code */
        if(mode === Mode.SURVEY && newMode !== Mode.SURVEY) {
            showMessageBoxSet(false);
            pie.showMessageTextSet(false);
        }
        /* End New Code */
        ...
        pie = elroi($container, [{series: series, options: {type:'pie'}}],
            {colors: ['#dddddd','#dddddd','#dddddd','#dddddd','#dddddd','#dddddd'],
                pie: {
                    ...
                    innerRadius: STANDARD_RADIUS *.58
                }
            });
